...
summary( cts.use$minority <- factor( ifelse( cts.use$race == 'White/Caucasian', 0, 1 ),
                         levels = 0:1,
                         labels = c('Non-Hispanic White/Caucasian','Minority Race/Ethnicity') )
)

summary( cts.use$nocollege <- factor( ifelse( cts.use$parenteduc == 'College degree' | cts.use$parenteduc == 'Post-college degree', 0, 1 ),
                         levels = 0:1,
...
#### Compare models with & without txcur*weeks effect
...
#### Compare models with & without procrastct*weeks
...
#### Compare models with & without bisct*weeks interaction
...
m6.symp: Drop interaction with current treatment
```{r}
...
#### Compare models with & without student stauts by time interaction
...
p <- plot( hm2.symppred ) +
  labs( title = 'Predicted Depression Score by Living Situation',
        y = 'PHQ-9 Total Score',
        x = 'Age Group (Years)',
